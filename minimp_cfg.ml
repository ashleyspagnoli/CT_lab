...
(* Alias for unannotated CFGs *)
type plain_cfg = unit cfg
...
let pp_cfg ?(pp_ann = fun _ -> "") (g : 'a cfg) : string =
  ...
  List.iter (fun id ->
    ...
    Buffer.add_string buf
      (Printf.sprintf "  [%d]  code: %-40s  next: %s%s\n" id (pp_block n.code) (pp_next n.next) (pp_ann n.ann))
  ) ids;
  Buffer.contents buf

(** CFG builder (produces a plain_cfg, annotation = unit) *)
...
(* Allocate a new node *)
let make_node (nodes : (int, unit node) Hashtbl.t) (code : block) (next : next) : int =
  let id = next_id () in
  Hashtbl.add nodes id { id; code; next; ann = () };
  id

(* Set the next field of an existing node *)
let set_next (nodes : (int, 'a node) Hashtbl.t) (id : int) (nxt : next) : unit =
...
let is_plain (n : 'a node) : bool =
  n.next = End &&
  not (List.exists (function SGuard _ -> true | _ -> false) n.code)

(* Replace every occurrence of old_id with new_id in all edges *)
let redirect_all (nodes : (int, 'a node) Hashtbl.t) (old_id : int) (new_id : int) : unit =
  let replace_next = function
    | Next x when x = old_id -> Next new_id
    | Branch (t, f) ->  Branch((if t = old_id then new_id else t), (if f = old_id then new_id else f))
    | other -> other
  in
  Hashtbl.iter (fun id n ->
    let nxt' = replace_next n.next in
    if nxt' <> n.next then Hashtbl.replace nodes id { n with next = nxt' }
  ) nodes

(** Commands Control Flow Graph *)

let rec cfg_of_cmd (nodes : (int, unit node) Hashtbl.t) (c : cmd) : int * int =
  match c with
  | Skip ->
...
and merge_seq nodes i1 f1 i2 f2 =
  ...
  if is_plain n_f1 && is_plain n_i2 then begin
    ...
    if f1_clean <> n_f1.code then
      Hashtbl.replace nodes f1
        { n_f1 with code = (if f1_clean = [] then [SSkip] else f1_clean) };
    set_next nodes f1 (Next i2);
    (i1, f2)
  end

let cfg_of_program (prog : program) : plain_cfg =
  let nodes = Hashtbl.create 16 in
  let (entry, exit_) = cfg_of_cmd nodes prog.body in
  { nodes; entry; exit = exit_ }

(** Helpers for building annotated CFGs from plain ones *)

(* Copy a plain CFG and attach a default annotation to every node *)
let annotate_cfg (default : 'a) (g : plain_cfg) : 'a cfg =
  let nodes2 : (int, 'a node) Hashtbl.t = Hashtbl.create (Hashtbl.length g.nodes) in
  Hashtbl.iter (fun id n ->
    Hashtbl.add nodes2 id { id = n.id; code = n.code; next = n.next; ann = default }
  ) g.nodes;
  { nodes = nodes2; entry = g.entry; exit = g.exit }

(* Return a sorted list of node ids *)
let sorted_ids (g : 'a cfg) : int list =
  Hashtbl.fold (fun id _ acc -> id :: acc) g.nodes []
  |> List.sort compare

(* Predecessors of a node (computed on demand) *)
let predecessors (g : 'a cfg) (id : int) : int list =
  Hashtbl.fold (fun pid n acc ->
    match n.next with
    | Next x when x = id -> pid :: acc
    | Branch (t, f) when t = id || f = id -> pid :: acc
    | _ -> acc
  ) g.nodes []

(* Successors of a node *)
let successors (g : 'a cfg) (id : int) : int list =
  match (Hashtbl.find g.nodes id).next with
  | End -> []
  | Next x -> [x]
  | Branch (t, f) -> [t; f]